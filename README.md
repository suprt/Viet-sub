...
**Auto-detect language** (for songs that start in English then switch to Russian — keeps the English intro):

```bash
python ru_to_vi_subtitles.py "path\to\video.mp4" --auto-language
```
...
3. **Transcribe** — faster-whisper `large-v3` on CUDA (Russian by default). VAD is **off by default** so the whole clip is transcribed. Use `--vad` for voice-activity filtering; use `--auto-language` for songs that start in English then Russian (keeps English intro). With `--save-transcription`, the raw transcription is written to a `.txt` file next to the output video.
